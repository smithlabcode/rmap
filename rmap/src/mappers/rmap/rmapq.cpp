/*    rmapq: a program for mapping Solexa reads using quality scores
...
map_reads(const string &chrom, const size_t chrom_id,
	  ...
	  const size_t max_diffs, const vector<FastReadQuality> &fast_reads,
...
process_score_data(const vector<vector<vector<double> > > &scores, 
  ...
  FastReadQuality::set_read_properties(read_width, 0, max_quality_score - min_quality_score);
...
void
adjust_prb_matrices(const double max_quality_score, 
...
main(int argc, const char **argv) {
  try {
    ...
    OptionParser opt_parse("rmapq",
			   "The rmapq mapping tool for Solexa reads"
			   " making use of base-call quality scores");
    ...
    adjust_prb_matrices(max_quality_score, scores);
    ...
    vector<FastReadQuality> fast_reads;
    for (size_t i = 0; i < reads.size(); ++i) {
      fast_reads.push_back(FastReadQuality(scores[i]));
      scores[i].clear();
    }
