/*    maskmers: program for masking sequences at k-mers that appear in
 *    a given file (special binary format)
 *
...
main(int argc, const char **argv) {
  ...
  try {
    ...
    OptionParser opt_parse("maskmers", "program for masking sequences "
			   " at k-mers that appear in a given file "
	...
	const string::const_iterator seq_start(sequences[i].begin());
	const string::const_iterator lim(sequences[i].end() - kmer + 1);
	for (string::const_iterator j(seq_start); j < lim; ++j) {
	  bool good = true;
	  ...
	  if (good) {
	    if (ambigs.find(long_index(j, j + kmer)) != ambigs.end() || 
		ambigs.find(long_index(j, j + kmer, true)) != ambigs.end())
