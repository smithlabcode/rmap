/*    occursonce: program for building library of all k-mers
 *    occurring once in a genome
 *
...
main(int argc, const char **argv) {
  ...
  try {
    ...
    OptionParser opt_parse("occursonce", "program for building library "
			   "of all k-mers occurring once in a genome.",
			   "<fasta-chrom-file>");
	...
	bool good = true;
 	for (size_t k = 0; k < static_cast<size_t>(kmer) && good; ++k)
	  ...
	  reads.insert(key);
 	  ...
 	  if (key != key_rc) reads.insert(key_rc);
 	}
      }
    }
    ...
    if (WRITE_TEXT) {
      ...
      for (unordered_set<long_index>::iterator i = reads.begin(); i != reads.end(); ++i)
