...
check_and_add(string &read, const int max_diffs,
  ...
  if (read_width == 0) {
    if (half_width*2 != read.length())
      throw RMAPException("PE reads not even length (must specify length)");
...
check_and_add(const FASTQScoreType score_format, const size_t max_diffs,
  ...
  const size_t half_width = (parts.size()/2)/rmap::alphabet_size;
  if (read_width == 0) {
    if (half_width*2 != parts.size()/rmap::alphabet_size)
      throw RMAPException("PE reads not even length (must specify length)");
    read_width = half_width;
  }
  else if (parts.size()/rmap::alphabet_size < 2*read_width)
    throw RMAPException("Incorrect read width");
  if (read_count == 0)
    FastReadWC::set_read_width(read_width);
  
  // convert to numerical values
  const size_t lim = parts.size()/rmap::alphabet_size;
  vector<vector<double> > error_probs(lim, vector<double>(rmap::alphabet_size));
