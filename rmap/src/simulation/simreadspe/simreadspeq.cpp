...
static void
simreadspeq(const Runif &rng, const size_t n_reads, const size_t read_width, 
  ...
  for (size_t i = 0; i < n_reads; ++i) {
    ...
    transform(seq_left.begin(), seq_left.end(),
	      seq_left.begin(), std::ptr_fun(&toupper));
    
    string seq_right = 
      revcomp(sequence.substr(start + separation, read_width));
    transform(seq_right.begin(), seq_right.end(), 
	      seq_right.begin(), std::ptr_fun(&toupper));
    
    const bool rc = (rng.runif(0.0,1.0) > 0.5);
    if (rc)
      seq_left.swap(seq_right);
    
    vector<vector<double> > errors_left;
    generate_sequencing_errors(rng, read_width, max_errors, errors_left);
    vector<vector<double> > quality_scores_left;
    seq_to_quality_scores(seq_left, quality_scores_left);
    add_sequencing_errors(errors_left, quality_scores_left);
    prob_to_quality_scores_solexa(quality_scores_left, quality_scores_left);
    
    vector<vector<double> > errors_right;
    generate_sequencing_errors(rng, read_width, max_errors, errors_right);
    vector<vector<double> > quality_scores_right;
    seq_to_quality_scores(seq_right, quality_scores_right);
    add_sequencing_errors(errors_right, quality_scores_right);
    prob_to_quality_scores_solexa(quality_scores_right, quality_scores_right);
    
    vector<vector<double> > quality_scores(quality_scores_left);
    quality_scores.insert(quality_scores.end(), quality_scores_right.begin(),
			  quality_scores_right.end());
    ...
    const string seq(seq_left + seq_right);
...
main(int argc, const char **argv) {
  ...
  try {
	...
	simreadspeq(rng, samples[i], read_width, separation,
		    max_errors, name, sequences[j], read_names, 
		    reads, probs);
      }
    }
