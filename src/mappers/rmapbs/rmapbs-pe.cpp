    ...
    vector<MultiMapResult> &best_maps)
{
    ...
    load_reads(VERBOSE, INPUT_MODE, RUN_MODE, AG_WILDCARD,
               max_mismatches, adaptor, reads_file, prb_file, 
                        ...
                        const size_t mismatch, MappedRead &mr)
{
    ...
    mr.r = GenomicRegion(MapResult::chrom_names[chrom_id], start, end,
    ...
    while (t_curr_idx < t_read_index.size() && a_curr_idx < a_read_index.size())
    {
        ...
        while (a_read_idx != a_read_index[a_curr_idx] && a_in.good())
        {
                ...
                && ReadOrderChecker()(prev_a_name, a_name))
            {
                ...
                throw SMITHLABException("Error reading " + A_reads_file);
        }
        ...
        if (same_read(suffix_len, t_name, a_name))
        {
            ...
            else
            {
                if (t_best_maps[t_curr_idx].mr.size() == 1)
                {
                    MappedRead one;
                    MapResult_to_MappedRead(
                        t_best_maps[t_curr_idx].mr.front(),
                        t_name, t_seq, t_scr, t_best_maps[t_curr_idx].score, one);
                    out << one << endl;
                }
                ...
                if (a_best_maps[a_curr_idx].mr.size() == 1)
                {
                    MappedRead two;
                    MapResult_to_MappedRead(
                        a_best_maps[a_curr_idx].mr.front(),
                        a_name, a_seq, a_scr, a_best_maps[a_curr_idx].score, two);
                    out << two << endl;
                }
            }
...
main(int argc, const char **argv) 
{
    try 
    {
        ...
        map_reads(chrom_file, filenames_file, fasta_suffix,
                  T_reads_file, prb_file, adaptor_sequence, n_seeds, seed_weight,
        ...
        map_reads(chrom_file, filenames_file, fasta_suffix,
                  A_reads_file, prb_file, adaptor_sequence, n_seeds, seed_weight,
                  read_width, max_mismatches, wildcard_cutoff,
                  FASTER_MODE, QUALITY, ALLOW_METH_BIAS,
                  WILDCARD, WILD_N_MODE, ORIGINAL_OUTPUT, VERBOSE,
