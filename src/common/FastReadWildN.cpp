...
FastReadWildN::WordPair::WordPair(const string &s) : 
...
char
FastReadWildN::WordPair::get_char(size_t mask, size_t pos) const {
  // 00 -> A, 01 -> C, 10 -> G, 11 -> T
...
string
FastReadWildN::WordPair::bits2string(size_t mask, size_t bits) {
  string s;
...
string
FastReadWildN::WordPair::tostring_bits(size_t mask) const {
...
string
FastReadWildN::WordPair::tostring_bases(size_t mask) const {
  string seq;
  for (size_t i = rmap_bits::word_size; i > 0; --i)
    seq += get_char(mask, i);
  return seq;
...
size_t FastReadWildN::score_mask = 0;
size_t FastReadWildN::segments = 0;
size_t FastReadWildN::read_width = 0;
size_t FastReadWildN::right_most_bit = 0;

void
FastReadWildN::set_read_width(const size_t m) {
  read_width = m;
...
FastReadWildN::FastReadWildN(const std::string &s_) {
  assert(s_.length() > 0);
...
FastReadWildN::FastReadWildN(std::string::const_iterator a,
...
string
FastReadWildN::tostring_bases() const {
...
string
FastReadWildN::tostring_bits() const {
  std::ostringstream ss;
  for (size_t i = 0; i < segments; ++i)
