...
#ifndef FAST_READ_WC_HPP
#define FAST_READ_WC_HPP
...
inline 
std::ostream& 
operator<<(std::ostream& s, const FastReadWC& fr) {
  return s << fr.tostring();
}

inline void
FastReadWC::Words::shift(const FastReadWC::Words &other) {
  a_vec <<= n_val_bits;
  c_vec <<= n_val_bits;
  g_vec <<= n_val_bits;
  t_vec <<= n_val_bits;
  a_vec |= ((other.a_vec & FastReadWC::high_val_bits) >> high_val_bits_to_low_val_bits_shift);
  c_vec |= ((other.c_vec & FastReadWC::high_val_bits) >> high_val_bits_to_low_val_bits_shift);
  g_vec |= ((other.g_vec & FastReadWC::high_val_bits) >> high_val_bits_to_low_val_bits_shift);
  t_vec |= ((other.t_vec & FastReadWC::high_val_bits) >> high_val_bits_to_low_val_bits_shift);
}

inline void
FastReadWC::Words::shift_last(const size_t i) {
...
inline void
FastReadWC::shift(const size_t idx) {
...
inline size_t
FastReadWC::score(const FastReadWC &other) const {
  size_t ss = 0;
...
inline size_t
FastReadWC::Words::score(const FastReadWC::Words &other, const size_t score_mask) const {
