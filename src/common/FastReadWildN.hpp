...
#ifndef FAST_READ_WILD_N_HPP
#define FAST_READ_WILD_N_HPP
...
class FastReadWildN {
public:
  FastReadWildN(const std::string &s);
  FastReadWildN(std::string::const_iterator a,
	   const std::string::const_iterator b);
  FastReadWildN() {
    wp.resize(segments + 1);
    wp[segments] = WordPair((rmap_bits::all_ones << right_most_bit));
  }
  bool operator<(const FastReadWildN &rhs) const {
  ...
  size_t score(const FastReadWildN &other) const;
...
inline 
std::ostream& 
operator<<(std::ostream& s, const FastReadWildN& fr) {
  return s << fr.tostring_bits();
}


inline void
FastReadWildN::WordPair::shift(const size_t i) {
...
inline void
FastReadWildN::WordPair::shift(const size_t i, const size_t shifter) {
...
inline void
FastReadWildN::shift(const size_t idx) {
  for (size_t i = 0; i < segments; ++i)
    wp[i].shift(wp[i + 1]);
...
inline size_t
FastReadWildN::score(const FastReadWildN &other) const {
  size_t ss = 0;
  for (size_t i = 0; i < segments; ++i)
...
inline size_t
FastReadWildN::WordPair::score(const FastReadWildN::WordPair &other, const size_t score_mask) const {
  // N is wildcard
...
inline void
FastReadWildN::WordPair::shift(const FastReadWildN::WordPair &other) {
